    ...
    if (len > 0 && filtered.length > 0) {
        for (i = 0; i < len; i = i + 1) {
            if (Object.prototype.hasOwnProperty.call(base, parts[i]) && isExtendableType(base[parts[i]])) {
                ...
                throw new TypeError(
                    'Could not access namespace \"' + sanitized + '\" while force mode is disabled. ' +
                    'All members in the chain must exist and be a truthy object or function.'
                );
...
[
    ...
    app = namespace(appName)  // a global object is returned and used as the forEach 'this'
);
