((define) => {define(() => {
    ...
    const namespace = (...rest) => {
        const defaultBase  = getTheGlobalObject() || {};
        const defaultForce = true;
        ...
        const base = isExtendableType(firstArg) ? firstArg : defaultBase;
        const force = typeof lastArg === 'boolean' ? lastArg : defaultForce;
        // Semantic fencepost for namespaces
        const separator = '.';
        // Match chains of at least one seperator
        const findSeparators = new RegExp('\\' + separator + '+', 'gi');
        const filtered = args.filter(function isUsableNamespace(item) {
    ...
    const stampObject = (keys, values) => {
        ...
        while (i < len) {
            ...
            const value = values[Math.max(Math.min(i, lastValueIndex), 0)];
